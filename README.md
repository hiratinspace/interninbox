...
<img src="https://raw.githubusercontent.com/hiratinspace/interninbox/main/assets/logo.png" alt="interninbox" width="440">
...
board APIs of **Greenhouse**, **Lever**, and **Ashby** — the same endpoints
each company's own careers page calls — plus, optionally, the official
**USAJOBS** API for federal Pathways internships.
...
- 🎯 **Search by role** — nine curated presets (`software`, `cybersecurity`, `finance`, …) or your own whole-word keywords.
...
1. **Internship signal** — word-boundary regexes on the title (`intern`, `internship`, `co-op`, `summer analyst`, `apprentice`, `student trainee`, …), OR any of your `include_keywords`. Word boundaries matter: *"International Program Manager"* and *"Internal Tools Engineer"* do **not** match.
...
adapter goes through — it's not a setting you can forget: sequential requests,
≥500 ms between same-host calls, a 15-second timeout, at most one retry (on
transient failures only), and an honest User-Agent. Only documented public
APIs are used — no HTML scraping, nothing behind a login.

## Scope, honestly

This tool does **one-shot local scans** — that's its whole job, done politely
...
Every test is offline — no recorded third-party data, ever
